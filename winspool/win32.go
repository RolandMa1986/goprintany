...
var (
	...
	startDocPrinterProc    = winspool.MustFindProc("StartDocPrinterW")
	startPagePrinterProc   = winspool.MustFindProc("StartPagePrinter")
	writePrinterProc       = winspool.MustFindProc("WritePrinter")
	endPagePrinterProc     = winspool.MustFindProc("EndPagePrinter")
	endDocPrinterProc      = winspool.MustFindProc("EndDocPrinter")
	...
	createCompatibleDCProc = gdi32.MustFindProc("CreateCompatibleDC")
	selectObjectProc       = gdi32.MustFindProc("SelectObject")
	bitBltProc             = gdi32.MustFindProc("BitBlt")
	stretchBltProc         = gdi32.MustFindProc("StretchBlt")
...
func (hPrinter HANDLE) StartDocPrinter(level uint32, docName, docType string) (int32, error) {
	var docInfo DocInfo
	var err error
	...
	docInfo.lpszDatatype, err = syscall.UTF16PtrFromString(docType)
	if err != nil {
		return 0, err
	}
	// docInfo.lpszOutput, err = syscall.UTF16PtrFromString("test.pdf")
	// if err != nil {
	// 	return 0, err
	// }

	r1, _, err := startDocPrinterProc.Call(uintptr(hPrinter), uintptr(level), uintptr(unsafe.Pointer(&docInfo)))
	if r1 == 0 {
		return 0, err
	}
	return int32(r1), nil
...
func (hDC HDC) CreateCompatibleDC() (HDC, error) {
	r1, _, err := createCompatibleDCProc.Call(uintptr(hDC))
	if r1 == 0 {
		return 0, err
	}
	return HDC(r1), nil
...
func (hDC HDC) SelectObject(hGDIObj HGDIOBJ) (HGDIOBJ, error) {
	r1, _, err := selectObjectProc.Call(uintptr(hDC), uintptr(hGDIObj))
	if r1 == 0 {
		return 0, err
	}
...
func (hDC HDC) BitBlt(x, y, width, height int32, src HDC, srcX, srcY int32, rop int32) error {
	r1, _, err := bitBltProc.Call(uintptr(hDC), uintptr(x), uintptr(y), uintptr(width), uintptr(height), uintptr(src), uintptr(srcX), uintptr(srcY), uintptr(rop))
	if r1 == 0 {
		return err
	}
	return nil
...
func (hDC HDC) StretchBlt(x, y, width, height int32, src HDC, srcX, srcY, srcWidth, srcHeight int32, rop int32) error {
	r1, _, err := stretchBltProc.Call(uintptr(hDC), uintptr(x), uintptr(y), uintptr(width), uintptr(height), uintptr(src), uintptr(srcX), uintptr(srcY), uintptr(srcWidth), uintptr(srcHeight), uintptr(rop))
	if r1 == 0 {
		return err
	}
	return nil
